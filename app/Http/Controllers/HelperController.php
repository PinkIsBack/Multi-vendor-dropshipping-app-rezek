...
class HelperController extends Controller
{
    ...
    public function finance_log($order = null,$finance =null,$status,$description = null,$tracking = null){
     $title = '';
        if($status == 1){
            $title = "Order Paid";
            $description = "Order ".$order->admin_shopify_name." has been paid amount of ".$order->supplier_pay." ZAR at ".now();
        }
        if($status == 2){
        ...
        if($status == 3){
            ...
            $description = "Order ".$order->admin_shopify_name." has been shipped with amount of ".$tracking->cost.' ZAR';
        }
        if($status == 4){
            $title = "Shipping Updated";
            $description = "Shipping amount set as ".$tracking->cost_shipping.' ZAR at '.now();
        }
        if($status == 5){
            $title = "Admin Paid";
